...
reference (correct hierarchy / joints / info, placeholder geometry, no skinCluster).
Vertex and skin data cannot be hand-authored reliably, so this script is the
authoritative generator for the *production* version: it builds real geometry,
smooth-binds it to the ``cloth_*`` joints, populates ``cloth_info``, and exports a clean
Maya ASCII asset. (The runtime fit rig was retired in M12 — see ``_build_controls``.)
...
def _ensure_package_importable() -> None:
    ...
    when run as an imported module; pasted straight into the Script Editor it can't be
    located and the build already tells the user to import it instead (see
    ``_default_out_path``).
    """
    try:
...
def _build_controls() -> str:
    # secondary animator control (stays accessible after attach). The runtime fit rig
    # (cloth_fit_ctrl + lattice) was retired in M12 — production ships pre-fit male/female
    # garment meshes instead of a runtime fit layer — so the example is now a plain
    # skinned garment with just this hand-animatable coat-tail control.
...
def _organize(meshes, tail) -> None:
...
def _delete_rig(export_grp: str) -> None:
    ...
    the skinned meshes cleanly, where ``exportSelected`` corrupts the deformer chain
    ('invalid components' / data-loss on re-import).
...
def build(out_path: str | None = None) -> str:
    ...
    _ensure_package_importable()  # the publish sidecar/thumbnail uses the shared core
    _clear_previous_build()
    export_grp = _build_skeleton_from_rig()
    meshes = _build_geometry()
    _bind(meshes)
    tail = _build_controls()
    _build_info_node()
    _organize(meshes, tail)
